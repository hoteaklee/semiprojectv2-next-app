import bcrypt from 'bcrypt'; // 설치


const saltRounds = 10; // salt키 생성 횟수 지정

// 압호 임력시 해시함수에 의해 해시코드로 변환된 암호 생성
...
export default async (req,res) => {
    let [passwd1, passwd2] = ['asd123', '987xyz'];
    let hashed1='', hashed2='';
    try {
        hashed1 = await hashPassword(passwd1);
        hashed2 = await hashPassword(passwd2);

        let isMatch1 =  await comparePasswd('asd123',  hashed1);
    ...
    return(
        <>
            <div>asd123의 해시코드 : {hashed1}</div>
            <div>987xyz의 해시코드 : {hashed2}</div>
        </>
    );
}